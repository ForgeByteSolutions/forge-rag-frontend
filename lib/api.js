...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    // --- Smart Routing Wake-up Logic ---
    if (
      (error.response?.status === 503 || error.response?.status === 504 || error.message === "Network Error" || error.code === "ECONNABORTED") &&
      !originalRequest._isWakeRetry
    ) {
      originalRequest._isWakeRetry = true;
      console.log("System is idle. Triggering auto-wake...");
      
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('backend-wakeup-start'));
      }

      fetch(`${process.env.NEXT_PUBLIC_API_BASE}/wake`, { method: "POST" }).catch(() => {});

      return new Promise((resolve, reject) => {
        const timer = setInterval(async () => {
          try {
            const check = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/health`);
            if (check.ok) {
              clearInterval(timer);
              if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('backend-wakeup-end'));
              }
...
export async function streamChat({
  ...
  try {
    ...
    if (err.message === "BACKEND_SLEEPING" || err.name === "TypeError") {
      console.log("System is idle. Triggering auto-wake...");
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('backend-wakeup-start'));
      }
        ...
        const timer = setInterval(async () => {
          try {
            ...
            if (check.ok) {
              ...
              // Retry streamChat with exact same parameters
              streamChat({ question, doc_id, workspace_id, doc_ids, model, onToken, onCitations, onEnd, signal })
                .then(resolve)
                .catch(reject);
            }
          } catch (e) { /* waiting */ }
